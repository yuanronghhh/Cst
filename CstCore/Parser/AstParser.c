...
/* Node Ast */
static SysBool ast_string_array_flatten (SysPtrArray *v_array, 
  const SysChar *key, 
  SysChar ***o_base, 
  SysUInt *o_len) {
...
static SysBool node_parse_prop_func(JNode *jnode, AstNodePass *pass) {
  ...
  switch (prop) {
    ...
    case CST_NODE_PROP_BASE:
      ...
      if(!cst_node_builder_parse_base(o, v_component, (const SysChar **)o_base, o_len)) {
        return false;
      }
    ...
    case CST_NODE_PROP_VALUE:
      ...
      if(!cst_node_builder_parse_value_bind(o, v_component, pair->key, (const SysChar *)nnode->v.v_string)) {
        return false;
      }
      break;
    case CST_NODE_PROP_POSITION:
      ...
      if(!cst_node_builder_parse_layer_name(o, nnode->v.v_string)) {
        return false;
      }
      break;
    case CST_NODE_PROP_ACTION:
      ...
      if(!cst_node_builder_parse_action(o, v_module, v_component, pair->key + 1, nnode->v.v_string)) {
        return false;
      }
      break;
...
CstCssPair *ast_css_pair_parse(JNode *jnode) {
  ...
  vnode = ast_jnode_jnode(jpair->value);
  if (!ast_css_value_parse(vnode, node, value)) {
