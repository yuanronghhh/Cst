...
void cst_node_map_construct(SysObject *o, CstPropMap *prop_map, SysInt prop_type) {
...
CstNodeMap* cst_node_map_new_I(CstPropMap *prop_map, SysInt prop_type) {
  ...
  cst_node_map_construct(SYS_OBJECT(o), prop_map, prop_type);
...
void cst_node_map_bind(CstNodeMap *self, CstComNode *com_node, CstNode *v_node) {
  ...
  if (priv->func) {
    priv->func(v_node, key, priv->value);
  }
}
