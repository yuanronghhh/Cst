...
static CstRenderNode* cst_com_node_realize_i(CstNode* o,
    CstRenderNode *v_parent, 
    CstComNode *com_node,
    CstLayout *layout) {

  CstComNode *self = CST_COM_NODE(o);
  CstRenderNode* rnode = CST_NODE_CLASS(cst_com_node_parent_class)->realize(
      o,
      v_parent, 
      self, 
      layout);

  return cst_component_realize(self->component, rnode, self, layout);
}
