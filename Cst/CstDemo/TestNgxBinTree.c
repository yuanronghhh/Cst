...
void *ngx_rbtree_lookup(ngx_rbtree_t *tree, ngx_rbtree_node_t *sentinel, char *name, signed int hash) 
{
  int                rc;
  void  *file;
  ngx_rbtree_node_t       *node;
  ...
  while (node != sentinel) {
    ...
    file = node;
    rc = tree->key_compare(name, file, tree->user_data);
    if (rc == 0) {
      return file;
    }
...
void ngx_rbtree_init(ngx_rbtree_t *tree, ngx_rbtree_node_t *sentinel,
  ngx_rbtree_insert_pt insert_pt,
  ngx_compare_pt key_compare,
  void *user_data) {

  ngx_rbtree_sentinel_init(sentinel);
  tree->root = sentinel;
  tree->sentinel = sentinel;
  tree->insert = insert_pt;
