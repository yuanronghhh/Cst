...
CstRenderNode* cst_node_realize(CstNode *self,
  ...
  return ncls->realize(self, v_parent, com_node, layout);
}
...
CstRenderNode *cst_node_new_render_node(CstNode* self) {
  ...
  if (self->rctx_type == 0) {
    sys_warning_N("%s", "node must be set render_context type before realize.");
    return NULL;
  }
...
static CstRenderNode* cst_node_realize_i(CstNode* self,
    CstRenderNode *v_parent, 
    CstComNode *com_node, 
    CstLayout *layout) {
  sys_return_val_if_fail(self != NULL, NULL);
  ...
  surface = cst_layout_get_default_surface(layout);
...
CstRenderNode* cst_node_realize_r(CstNode *self,
  ...
  CstRenderNode *rnode;
  CstNode *co, *no;
  rnode = cst_node_realize(self, v_parent, com_node, layout);

  co = cst_node_get_children(self);
  if (co) {

    cst_node_realize_r(co, rnode, com_node, layout);
  }

  no = cst_node_get_next(self);
  if (no) {

    cst_node_realize_r(no, v_parent, com_node, layout);
  }
...
static void cst_node_init(CstNode *self) {
  ...
  /* default flex now, maybe change. */
  self->rctx_type = CST_TYPE_FLEX_CONTEXT;
  self->alg_type = CST_TYPE_FLEX_ALGORITHM; 
  self->rnode_type = CST_TYPE_LBOX;
}
