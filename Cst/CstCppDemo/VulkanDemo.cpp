...
namespace DebugMarker
{
  ...
  void beginRegion(VkCommandBuffer cmdbuffer, const char* pMarkerName, vec4 color)
  {
    // Check for valid function pointer (may not be present if not running in a debugging application)
    if (active)
    {
  ...
  void insert(VkCommandBuffer cmdbuffer, std::string markerName, vec4 color)
  {
    // Check for valid function pointer (may not be present if not running in a debugging application)
    if (active)
    {
...
struct Scene {
  ...
  void draw(VkCommandBuffer cmdBuffer)
  {
    ...
    for (auto i = 0; i < model.nodes.size(); i++)
    {
      ...
      DebugMarker::insert(cmdBuffer, "Draw \"" + model.nodes[i]->name + "\"", (vec4) { 0.0f });
...
public:
  ...
  struct UBOVS {
    mat4 projection;
    mat4 model;
    vec4 lightPos = vec4(0.0f, 5.0f, 15.0f, 1.0f);
  } uboVS;
  ...
  VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION)
  {
    ...
    camera.setRotation(vec3(-4.35f, 16.25f, 0.0f));
    camera.setRotationSpeed(0.5f);
    camera.setPosition(vec3(0.1f, 1.1f, -8.5f));
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
    {
      ...
      */
      if (glow)
      {
        ...
        // Start a new debug marker region
        DebugMarker::beginRegion(drawCmdBuffers[i], "Off-screen scene rendering", vec4(1.0f, 0.78f, 0.05f, 1.0f));
      ...
      */
      {
        ...
        // Start a new debug marker region
        DebugMarker::beginRegion(drawCmdBuffers[i], "Render scene", vec4(0.5f, 0.76f, 0.34f, 1.0f));
        ...
        // Start a new debug marker region
        DebugMarker::beginRegion(drawCmdBuffers[i], "Toon shading draw", vec4(0.78f, 0.74f, 0.9f, 1.0f));
        ...
        {
          // Insert debug marker
          DebugMarker::beginRegion(drawCmdBuffers[i], "Wireframe draw", vec4(0.53f, 0.78f, 0.91f, 1.0f));
        ...
        if (glow)
        {
          DebugMarker::beginRegion(drawCmdBuffers[i], "Apply post processing", vec4(0.93f, 0.89f, 0.69f, 1.0f));
...
void test_vulkan_demo_run(void) {
}
