...
#define BUFSIZE 255
#define DEMO_FILE "D:/GreyHound/PRIVATE/DOWNLOADS/DOWNLOADS.7z"
gpointer func1(gpointer data) {
  sys_debug_N("%s", "[func1]");
  gchar *path = DEMO_FILE;
  guint buf[BUFSIZE];
  gsize sz;
  ...
  while (1) {
    sz = fread(&buf[0], sizeof(guint), BUFSIZE, fp);
    if (sz == 0) {
      break;
    }

    g_coroutine_yield(&data);
  }

  fclose(fp);
  sys_debug_N("%s", "[func1 Done]");
...
gpointer func2(gpointer data) {
  ...
  sys_debug_N("%s", "[func2 Done]");
  return NULL;
}

gpointer func3(gpointer data) {
  sys_debug_N("%s", "[func3]");
  gchar *path = DEMO_FILE;
  guint buf[BUFSIZE];
  gsize sz;
  ...
  fclose(fp);
  sys_debug_N("%s", "[func3 Done]");

  return NULL;
}

gpointer func4(gpointer data) {
  sys_debug_N("%s", "[func4]");
  gchar *path = DEMO_FILE;
  guint buf[BUFSIZE];
  gsize sz;
  ...
  sys_debug_N("%s", "[func4 Done]");
  return NULL;
}

void test_coroutine_async1(void) {
  GCoroutine *co = g_coroutine_new(func1);
  GCoroutine *co2 = g_coroutine_new(func2);

  SysInt i = 0;
  gpointer done1, done2;
  gint64 start;
  gint64 end;

  start = g_get_monotonic_time();
  while (true) {
    if (g_coroutine_resumable(co)) {
      done1 = g_coroutine_resume(co, NULL);
    }

    if (g_coroutine_resumable(co2)) {
      done2 = g_coroutine_resume(co2, NULL);
    }
  ...
  g_coroutine_unref(co2);

  sys_debug_N("%d,%d,Done", i, end - start);
}

void test_coroutine_async2(void) {
  GCoroutine *co = g_coroutine_new(func3);
  GCoroutine *co2 = g_coroutine_new(func4);
  ...
  g_coroutine_resume(co2, NULL);
  end = g_get_monotonic_time();

  g_coroutine_unref(co);
  g_coroutine_unref(co2);

  sys_debug_N("%d,Done", end - start);
}

int test_coroutine_init(int argc, char **argv)
{
  SYS_LEAK_IGNORE_BEGIN
  // ignore global random int init.
  g_test_init (&argc, &argv, NULL);
  SYS_LEAK_IGNORE_END

  g_test_add_func("/basic/async1", test_coroutine_async1);
  // g_test_add_func("/basic/async2", test_coroutine_async2);
